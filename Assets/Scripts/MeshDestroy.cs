...
public class MeshDestroy : MonoBehaviour
{
    ...
    [HideInInspector] public bool thisShard = false;
    ...
    void Update()
    {
        if(!thisShard)
        {
            if (this.gameObject.GetComponent<TheSpace.SpaceObject>().exploded)
            {
                this.gameObject.GetComponent<TheSpace.SpaceObject>().exploded = false;
                DestroyMesh();
            }
        }
    }
    ...
    private void DestroyMesh()
    {
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                //var plane = new Plane(UnityEngine.Random.onUnitSphere, new Vector3(UnityEngine.Random.Range(bounds.min.x, bounds.max.x), UnityEngine.Random.Range(bounds.min.y, bounds.max.y), UnityEngine.Random.Range(bounds.min.z, bounds.max.z)));
                var plane = new Plane(UnityEngine.Random.onUnitSphere, new Vector3(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, bounds.max.z - bounds.min.z));
    ...
    public class PartMesh
    {
        ...
        {
            GameObject = new GameObject("ShardOf" + original.name);
